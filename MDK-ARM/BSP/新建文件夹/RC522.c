...
int ReaderCard(void)
{
	...
	if(PcdRequest(PICC_REQALL,Temp)==MI_OK)	//选卡
	{
		if(Temp[0]==0x04&&Temp[1]==0x00)  
				LCD_ShowText("MFOne-S50");
		else if(Temp[0]==0x02&&Temp[1]==0x00)
			LCD_ShowText("MFOne-S70");
		else if(Temp[0]==0x44&&Temp[1]==0x00)
			LCD_ShowText("MF-UltraLight");
		else if(Temp[0]==0x08&&Temp[1]==0x00)
			LCD_ShowText("MF-Pro");
		else if(Temp[0]==0x44&&Temp[1]==0x03)
			LCD_ShowText("MF Desire");
		else
			LCD_ShowText("Unknown");
		if(PcdAnticoll(UID)==MI_OK)			//防冲撞
		{ 
			sprintf((char*)display,"Card Id is:%d%d%d%d",UID[0],UID[1],UID[2],UID[3]);
			/* 获取卡值  */
			LCD_ShowText((char*)display); 		              
			temp_value = ((UID[0]>>4)*10+(UID[0]&0x0f));
			switch(temp_value)
			{
				case 10 : LCD_ShowText("Pass\r\n");
				          flag=1;
				          break;
				default : LCD_ShowText("Fail\r\n");    break;
			}	                             
		}
   }
   return flag;	
